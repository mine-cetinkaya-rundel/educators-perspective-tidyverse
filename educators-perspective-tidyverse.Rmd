...
abstract: >
  ...
  In this paper, we introduce the tidyverse from an educator's perspective. We provide a brief introduction to the tidyverse, demonstrate how foundational statistics and data science tasks are accomplised with the tidyverse, and discuss the strengths of the tidyverse, particularly in the context of teaching and learning.
...
The revised Guidelines for Assessment and Instruction in Statistics Education (GAISE) College report notes the importance of technology and states: "ideally, students should be given numerous opportunities to analyze data with the best available technology (preferably, statistical software)" [@CarEve2016].
In both statistics and data science courses, we believe it is important to teach authentic tools, i.e., tools that are used by practitioners of these disciplines (academics and industry professionals).

When it comes to an authentic tool, @mcna_2019 argues that a modern statistical computing tool "should be accessible, provide easy entry, privilege data as a first-order object, support exploratory and confirmatory analysis, allow for flexible plot creation, support randomization, be interactive, include inherent documentation, support narrative, publishing, and reproducibility, and be flexible to extensions," and that such tools ideally allow new users and professionals to "reach across the gap" between tools for teaching and tools for doing to foster continued learning [@mcnamara2015; @mcna_2019].
...
While we all recommend the use of the RStudio integrated development environment (IDE) [@rstudio], our arguments for the tidyverse stand even outside the RStudio IDE. 
...
Central to the tidyverse is the notion of tidy data, which @wickham2014tidy defines as "easy to manipulate, model and visualize, and have a specific structure: each variable is a column, each observation is a row, and each type of observational unit is a table."
...
Beginning with R 4.1, the previous `arrange()` code would be written as `loans |> arrange(loan_amount)` using the pipe operator (`|>`) to pipe the data frame (or any result from the preceding step of the pipeline) into the subsequent function as its first argument (see Section \ref{sec:wrangle} for more details on using the pipe operator in the tidyverse).
...
The widespread community adoption of the tidyverse (and the corresponding improved error recovery) led to the introduction of a pipe operator into the base language[^tierney] in early 2021 with R version 4.1.0.[^1]

[^1]: In versions of R earlier than 4.1.0, the magrittr pipe (`%>%`) must be used in place of the native pipe (`|>`). When using the base R pipe, note that *all* students must be running R version 4.1 or newer).
...
@hermans2017writing develop the metaphor of programming as writing, and we extend their reasoning to assert that R is a programming language with many syntaxes (or flavors/"dialects").
...
Tidyverse users tend to use particular vocabulary, e.g., pipes, tibbles, and verbs, compared to base R users who are more likely to speak in terms of matrices, dollar signs (`$`), and square brackets (`[]`).
...
```{r ggplot_ex, ref = "ggplot-exref", codecap = "Using ggplot2 to create boxplots of \\texttt{loan\\_amount} broken down by both \\texttt{application\\_type} and \\texttt{homeownership}.", fig.asp = 0.5}
loans |>
...
```{r base_plot_ex, ref = "base-plot-ex", codecap = "Using base R to create boxplots of \\texttt{loan\\_amount} broken down by both \\texttt{application\\_type} and \\texttt{homeownership}.", fig.asp = 0.5}
...
```{r ggplot_ex2, ref = "ggplot-ex2", codecap = "Using ggplot2 to create boxplots of \\texttt{loan\\_amount} broken down by both \\texttt{application\\_type} and \\texttt{homeownership}, filled by \\texttt{bankruptcy}.", fig.asp = 0.5, out.width = "90%"}
loans |>
...
```{r ggplot-scat, ref = "ggplot-scat", codecap = "Using ggplot2 to create a scatterplot of \\texttt{interest\\_rate} versus annual income, colored by \\texttt{bankruptcy}.", out.width = "80%", message = FALSE}
loans |>
...
```{r base-scat, ref = "base-scat", codecap = "Using base R to create a scatterplot of \\texttt{interest\\_rate} versus annual income, colored by \\texttt{bankruptcy}.", out.width = "80%"}
...
The ggplot2 approach uses function arguments vs. base graphics `par()` parameters.
...
However, one can certainly teach introductory statistics by combining the core tidyverse functionality (i.e., dplyr and ggplot2) for wrangling with base R implementations of inferential functions (e.g., `t.test()`, `chisq.test()`, etc.).
...
Which do you skip?
When we read the tidyverse code, we pronounce the `|>` operator as "and then." "Group by bankruptcy and then summarize." When we read the base code, it is more repetitive because we need to repeatedly say things like "loans dollar sign bankruptcy." Typically, we do not vocalize every character on the screen.
...
To fully drive the 1-to-1 equivalence home, **dbplyr** contains the function `show_query()` that will explicitly translate a dplyr pipeline to a SQL query.
